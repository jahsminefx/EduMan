...
export default function BulkUploadStudents() {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex flex-col gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100 md:flex-row md:items-center md:justify-between">
        <div>
          <Link to="/dashboard/admin/students" className="inline-flex items-center text-sm text-blue-600 font-medium mb-2 hover:text-blue-800">
            <ArrowLeft className="w-4 h-4 mr-1" />
            Students
          </Link>
          <h2 className="text-xl font-semibold text-gray-800">Bulk Upload Students</h2>
          <p className="text-sm text-gray-500">Import student records from a CSV file.</p>
        </div>
        <button
          type="button"
          onClick={downloadTemplate}
          className="inline-flex items-center justify-center px-4 py-2 bg-gray-800 text-white text-sm font-medium rounded-lg hover:bg-gray-900"
        >
      ...
      {message && (
        <div className={`p-4 rounded-xl flex items-start gap-3 border ${message.type === 'error' ? 'bg-red-50 text-red-800 border-red-100' : 'bg-green-50 text-green-800 border-green-100'}`}>
      ...
      <form onSubmit={handleUpload} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-5">
        <div className="border border-dashed border-gray-300 rounded-xl p-6 bg-gray-50">
          <div className="flex flex-col items-center text-center gap-3">
            ...
            <div>
              <label className="block text-sm font-bold text-gray-900 mb-2">CSV File</label>
              <input
                type="file"
                accept=".csv,text/csv"
                onChange={(event) => setFile(event.target.files?.[0] || null)}
                className="block w-full text-sm text-gray-700 file:mr-4 file:rounded-lg file:border-0 file:bg-blue-600 file:px-4 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-blue-700"
              />
            </div>
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            disabled={uploading}
            className="inline-flex items-center px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 disabled:opacity-60"
          >
