...
export default function AIContentGenerator() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 rounded-2xl border border-blue-100 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="flex items-center gap-2 text-xl font-bold text-gray-900"><Sparkles className="h-5 w-5 text-blue-600" /> EduMan AI Learning Content Generator</h2>
          <p className="mt-1 text-sm text-gray-500">Create an editable document, review it, then publish it to the learning library.</p>
        </div>
        {options.usage && (
          ...
          {options.assignments.length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <label className="text-sm font-medium text-gray-700">
                  Topic
                  <input required value={form.topic} onChange={event => setForm({ ...form, topic: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 p-2.5" placeholder="e.g. The Water Cycle" />
                </label>
                <label className="text-sm font-medium text-gray-700">
                  Content type
                  <select value={form.content_type} onChange={event => setForm({ ...form, content_type: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 bg-white p-2.5">
                    {contentTypeOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                  </select>
                </label>
                <label className="text-sm font-medium text-gray-700">
                  Length
                  <select value={form.length} onChange={event => setForm({ ...form, length: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 bg-white p-2.5">
                    <option value="short">Short</option><option value="medium">Medium</option><option value="long">Long</option>
                  </select>
                </label>
                <label className="text-sm font-medium text-gray-700">
                  Tone
                  <select value={form.tone} onChange={event => setForm({ ...form, tone: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 bg-white p-2.5">
                ...
                <div className="flex flex-col justify-end gap-3 rounded-lg border border-gray-200 p-3">
                  <label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={form.include_examples} onChange={event => setForm({ ...form, include_examples: event.target.checked })} /> Include examples</label>
                  <label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={form.include_assessment} onChange={event => setForm({ ...form, include_assessment: event.target.checked })} /> Include assessment questions</label>
                </div>
                <label className="text-sm font-medium text-gray-700">
                  Academic session
              ...
              <button disabled={busy || !options.ai_enabled} className="inline-flex items-center rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">
        ...
        <div className="space-y-5">
          <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <label className="md:col-span-2 text-sm font-medium text-gray-700">Document title<input value={resource.title} onChange={event => setResource({ ...resource, title: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 p-2.5" /></label>
              <label className="text-sm font-medium text-gray-700">Content type<select value={resource.content_type} onChange={event => setResource({ ...resource, content_type: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 bg-white p-2.5">{contentTypeOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
            </div>
            <label className="mt-5 block text-sm font-medium text-gray-700">
              Document body
              <textarea value={resource.body} onChange={event => setResource({ ...resource, body: event.target.value })} rows="28" className="mt-1 w-full rounded-xl border border-gray-300 p-4 font-mono text-sm leading-6" />
            </label>
          </div>
          ...
          <div className="sticky bottom-4 flex flex-wrap gap-3 rounded-2xl border border-gray-200 bg-white/95 p-4 shadow-lg backdrop-blur">
            ...
            <button onClick={regenerate} disabled={busy} className="inline-flex items-center rounded-xl border border-blue-300 px-4 py-2 text-sm font-bold text-blue-700 hover:bg-blue-50 disabled:opacity-60">{busy === 'regenerate' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-2 h-4 w-4" />} Regenerate</button>
            <button onClick={publish} disabled={busy} className="inline-flex items-center rounded-xl bg-green-600 px-4 py-2 text-sm font-bold text-white hover:bg-green-700 disabled:opacity-60">{resource.status === 'published' ? <CheckCircle className="mr-2 h-4 w-4" /> : <Send className="mr-2 h-4 w-4" />} {resource.status === 'published' ? 'Published' : 'Save & Publish'}</button>
            <button onClick={() => download('pdf')} disabled={busy} className="inline-flex items-center rounded-xl border border-gray-300 px-4 py-2 text-sm font-bold text-gray-700"><Download className="mr-2 h-4 w-4" /> PDF</button>
            <button onClick={() => download('docx')} disabled={busy} className="inline-flex items-center rounded-xl border border-gray-300 px-4 py-2 text-sm font-bold text-gray-700"><FileText className="mr-2 h-4 w-4" /> DOCX</button>
            {!draftId && <button onClick={() => setResource(null)} className="ml-auto text-sm font-semibold text-gray-500 hover:text-gray-800">Start another document</button>}
          </div>
        </div>
      )}
    </div>
