...
exports.submitAssignment = async (req, res) => {
    ...
    try {
        ...
        const assignment = await db.get('SELECT class_id, school_id FROM homework WHERE id = ?', [assignment_id]);
        ...
        const result = await db.run(
            'INSERT INTO assignment_submissions (assignment_id, student_id, file_path, file_type) VALUES (?, ?, ?, ?)',
...
exports.getSubmissions = async (req, res) => {
    ...
    try {
        ...
        const hw = await db.get('SELECT id FROM homework WHERE id = ? AND school_id = ?', [assignment_id, school_id]);
