    ...
    try {
        ...
        if (req.user.role === 'Teacher') {
            ...
            const assignment = await db.get(
                'SELECT id FROM teacher_subject_assignments WHERE teacher_id = ? AND class_id = ?',
                [teacher.id, class_id]
            );
            if (!assignment) {
        ...
        for (const record of records) {
            const existing = await db.get(
                'SELECT id FROM attendance_records WHERE student_id = ? AND date = ? AND class_id = ?',
                [record.student_id, date, class_id]
            );

            if (existing) {
                await db.run(
                    'UPDATE attendance_records SET status = ?, recorded_by = ? WHERE id = ?',
...
exports.getAttendance = async (req, res) => {
    ...
    try {
        ...
        const records = await db.all(`
            ...
            FROM students s
            LEFT JOIN attendance_records a ON s.id = a.student_id AND a.date = ? AND a.class_id = ?
