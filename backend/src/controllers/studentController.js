...
exports.updateStudent = async (req, res) => {
    ...
    try {
        ...
        await db.run(
            `UPDATE students 
             SET first_name=?, last_name=?, gender=?, dob=?, class_id=?, parent_name=?, parent_phone=? 
...
exports.deleteStudent = async (req, res) => {
    ...
    try {
        const db = getDB();
        const school_id = req.user.school_id;
        const result = await db.run(`DELETE FROM students WHERE id=? AND school_id=?`, [id, school_id]);
